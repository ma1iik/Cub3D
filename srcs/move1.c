...
/*   Updated: 2023/03/04 14:25:01 by ma1iik           ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void	move_up(t_data *data)
{
    ...
    if (data->r_distance < (sqrt((pow(data->pl_tx_x - i, 2) + pow(data->pl_tx_y - j, 2)))))
    {
        i = data->x_fnl;
        j = data->y_fnl;
    }
	...
	if (check_wall_around(data, i, j, 0) == 0 )
	{
		...
		data->pl_tx_x = i;
		data->pl_tx_y = j;
    // Redraw the screen with the new player position
    cast_rays1(data);
    }
}
...
{
    int start_x = fmax(0, i - radius);
    int start_y = fmax(0, j - radius);
    int end_x = fmin(WIDTH - 1, i + radius);
    int end_y = fmin(HEIGHT - 1, j + radius);
    int r_squared = radius * radius;

    for (int x = start_x; x <= end_x; x++) {
        for (int y = start_y; y <= end_y; y++) {
            if (pow(x - i, 2) + pow(y - j, 2) <= r_squared && data->map[y/TX_L][x/TX_L] == '1') {
                return 1;
            }
        }
    }
